...
def viralite_analysis(clips_raw, opts, raw_paths):
    ...
    Appelle l'API Claude via ANTHROPIC_API_KEY (env var GitHub Actions).
    ...
    if not api_key:
        print("  [Viralité] ANTHROPIC_API_KEY absent — analyse ignorée")
        return
...
{{
  ...
  "recs": [
    ...
    try:
        ...
        raw_text = "".join(b.get("text","") for b in body.get("content",[]))
